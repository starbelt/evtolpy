...
These are derived from the fundamental kinematic equations:
...
These relations are applied differently depending on the type of maneuver (e.g., takeoff, climb, cruise, descent, hover), which is one the reasons that leads to different average shaft power calculation for each segment.
...
**<u>Note:<u>**: Different calculation will be implemented for each mission segment.
...
* $v_{avg}$ = average horizontal velocity (*depart_taxi_avg_h_m_p_s*)  
* $t$ = duration of taxi segment (*depart_taxi_s*)  
