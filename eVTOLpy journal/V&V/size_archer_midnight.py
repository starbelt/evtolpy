...
json_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../example aircraft/Archer_Midnight.json'))
...
reference_values = {
    "MTOW (kg)": 3175.0,
    "Empty Weight (kg)": 1968.5,
    "Hover power (kW)": 0.0,
    "Cruise power (kW)": 0.0,
    "Battery weight (kg)": 0.0,
    "Cruise L/D": 0.0,
}
...
num_pax = 4
pax_km_per_kwh = (num_pax * cruise_distance_km / cruise_energy_kwh) if cruise_energy_kwh else None

sized_values = {
    "MTOW (kg)": ctow_kg,
    "Empty Weight (kg)": ac.empty_mass_kg,
